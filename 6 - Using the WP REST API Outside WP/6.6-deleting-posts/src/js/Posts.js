...
export default class Posts {
  static loadPosts(event) {
    ...
    axios
      .get(config.rest_url + "wp/v2/posts", {
        params: {
          per_page: 3,
        },
      })
  ...
  static loadSinglePost() {
    ...
    axios.get(config.rest_url + "wp/v2/posts/" + id).then((response) => {
      Helpers.clearPosts();
  ...
  static renderPost(post, single = false) {
    ...
    if (true === single) {
      markup +=
        '<p><a class="back-to-posts" href="#">&lt; Go Back to Posts</a>';
    ...
    if (true === single) {
      article
        .querySelector("a.back-to-posts")
        .addEventListener("click", Posts.loadPosts, false);
    } else {
      article
        .querySelector(".entry-title a")
        .addEventListener("click", Posts.loadSinglePost, false);
    }
